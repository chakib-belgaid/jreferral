...
TITLSEDIC={'duration':'Execution Time (s)','CPU':'Energy CPU (J)','DRAM':'Energy DRAM (J)','energy':'Total Energy (J)','jvm':'JVM','options':'Execution Flags'}
...
def read_tags():
    ...
    with open(configfile,'r') as f : 
        ...
        tags={x.split(' TAG ')[0]:x.split(' TAG ')[1].lstrip() for x in lines }
        tags['default'] = ''
        return tags 
...
def get_recap(x,metrics):
    ...
    recap=recap.set_index(["Type","Metric"]).loc[:,metrics+["jvm","options"]]
...
def plot_box(df,metric,axes):
   ...
   ############# inside the boxplots 
    ...
    axes.set_xticklabels([ ' '.join(x.get_text().split(' ')[1:])for x in axes.get_xticklabels()])
    return axes
...
def read_data(path):
    ...
    data["name"]=data.apply(lambda row : f"{(row['distribution'])} {row['jvm']} {get_tag(row['options'])} ",axis=1)
...
def main() : 
    ...
    metrics=['duration','CPU','DRAM'] if args.details else ['duration','energy']
    maxi=max(data["iteration"].unique())
    uniqueBenchmark=False


    if  not  "benchmark" in data.columns:
    ...
    if args.plot:
        ...
        filename= ""  if uniqueBenchmark else os.path.basename(args.path).replace(".csv","_")
...
def get_tag(options):
    ...
    try : 
        ...
        if not (tag in  undifinedOptions) :
            undifinedOptions.append(tag)
            logger.warning(f"""Warrning --  The options : [{options}] has no Tag, hence it will take the option as a tag.\nPlease consider adding an apropriate tag in the jvms-options-names.sh file""")
